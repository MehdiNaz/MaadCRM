...
public class CustomerActivityHistoryMapping : IEntityTypeConfiguration<CustomerActivityHistory>
{
    public void Configure(EntityTypeBuilder<CustomerActivityHistory> builder)
    {
        ...
        builder.HasOne(x => x.Customers).WithMany(x => x.CustomerActivityHistorys).HasForeignKey(x => x.CustomerId);
        builder.HasOne(x => x.CustomerActivity).WithMany(x => x.CustomerActivityHistories).HasForeignKey(x => x.CustomerActivityId);
    }
}